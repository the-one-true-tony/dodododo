...
class GameView{
  ...
  loadStartModal(){
    if(!this.startModal){
      ...
      instructions.innerHTML = "Lyrics Hero is a simple typing game.  Pick a song and press start. Letters will scroll from right to left. Once the letter reaches the box on the left press the corrisponding key.  If you've timed it correctly you'll receive points.  See how many points you can get!";
      this.startModal.appendChild(instructions);
  ...
  loadMenu(){
    ...
    const clickMuteButton = () => {
      if(this.game.startTime > 0){
        if (this.game.musicPlayer.muted === true) {
          this.game.musicPlayer.muted = false;
          muteButton.innerHTML = "Mute";
        } else {
          this.game.musicPlayer.muted = true;
    ...
    muteButton.className = "menuButton";
    muteButton.innerHTML = "Mute";
    menu.appendChild(muteButton);
    muteButton.addEventListener("click", clickMuteButton);
    
    const menuButton = document.createElement("button");
    menuButton.className = "menuButton";
    menuButton.innerHTML = "Menu/Pause";
